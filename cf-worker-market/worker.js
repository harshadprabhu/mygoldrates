...
// ─── /chart (and /intraday) ─────────────────────────────────────────────
// OHLC candles for the live Gold Spot chart, at any interval from 1 minute
// to 1 month. /ohlc is the older fixed 90-day daily endpoint and is kept
// for existing callers; this one takes interval + range so the page can
// offer a proper timeframe selector.
//
// gold-api.com (our spot source) publishes a single current price with no
// history at all, so the shape has to come from Yahoo's gold futures.
// Futures sit ~1% over spot on contango, so every bar is multiplied by
// (spot_now / futures_now) before being returned. That is a UNIFORM
// multiplicative shift: it preserves the shape of the series exactly and
// only moves the absolute level, keeping the chart's axis consistent with
// the Gold Spot tile instead of silently switching the reader to a
// futures price.
//
// Sub-minute resolution is not offered here because Yahoo has none. The
// page builds its own "LIVE" seconds series from the spot prices it
// actually observes on each poll.
...
async function handleChart(url) {
  ...
  const def = CHART_INTERVALS[interval];
  if (!def) return jsonErr(`unsupported interval: ${interval}`, 400);
  const range = url.searchParams.get('range') || def.range;

  // Intraday bars go stale in a minute; daily and longer do not.
  const intraday = interval.endsWith('m') && interval !== '1mo';
  const ttl = intraday ? 60 : 3600;

  const [raw, spotNow] = await Promise.all([
    safeFetchJson(
      `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(cfg.ticker)}?range=${range}&interval=${interval}`,
      { cacheTtl: ttl, timeoutMs: 12000 }),
    safeFetchJson(`https://api.gold-api.com/price/${cfg.spot}`, { cacheTtl: 30 }),
  ]);

  const result = raw?.chart?.result?.[0];
  if (!result) return jsonErr('chart upstream returned no result', 502);
  const meta = result.meta || {};
  const ts = result.timestamp || [];
  const q = result.indicators?.quote?.[0];
  if (!q) return jsonErr('chart upstream missing quote block', 502);

  const futNow = meta.regularMarketPrice;
  const spot = spotNow?.price;
  const scale = (spot > 0 && futNow > 0) ? spot / futNow : 1;
  const px = (v) => (v == null ? null : +(v * scale).toFixed(4));
  ...
  for (let i = 0; i < ts.length; i++) {
    ...
    // A bar missing o/h/l still carries a usable close, so fall back to it
    // rather than dropping the bar and leaving a gap in the line.
    candles.push({
      t: ts[i],
      o: px(q.open?.[i] ?? c),
      h: px(q.high?.[i] ?? c),
  ...
  return json({
    ...
    candles,
    points,          // kept so the previous /intraday shape still works
    updated_at: new Date().toISOString(),
  });
}
...
const ROUTES = {
  ...
  '/intraday': { ttl: 60,    fn: (req, url) => handleChart(url) },
  '/chart':    { ttl: 60,    fn: (req, url) => handleChart(url) },
